...
const InsulationSection = () => {
  ...
  const fetchFeaturedCategory = async () => {
    try {
      ...
      const featuredCat = categoriesResponse.results.find(
        (cat) => cat.feature === 'feature1'
      )

      if (!featuredCat) {
        throw new Error('No category with feature1 found')
      }
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // 2. Get products for this category (default view)
    ...
    try {
      ...
      switch (viewMode) {
        ...
        case 'popularProducts':
          ...
          break
        default:
          products = await fetchProducts(featuredCategory.id, page)
      }
      setProductsData(products)
      setCurrentPage(page)
    } finally {
    ...
    <div className="Container py-6 my-8">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Pagination - Only show for default view */}
          {viewMode === 'default' &&
            productsData.results.length > 0 &&
            totalPages > 1 && (
              <div className="mt-6">
